...
const Card = ({
  ...
  useEffect(() => {
    const fetchApplicationCount = async () => {
      try {
        const response = await fetch(`https://api.linksus.in/api/applications/count?taskId=${taskId}`);
        if (!response.ok) {
            ...
            <div className="domain-applicant">
              ...
              <div className="main-applicant">
                <div className="applicant">
                  ...
                  <p className="number-of-applicant">{applicationCount === 0 ? 2 : applicationCount} Applications</p>
                </div>
              </div>
            </div>
          </div>
